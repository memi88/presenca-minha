...
- [x] Checagem de sessão local na Entrada: sessão válida → pula onboarding, vai direto pra Home. `(marketing)/page.tsx`, `bem-vindo/page.tsx` e `login/page.tsx` checam `auth.getUser()` no servidor antes de renderizar.
- [x] Link discreto "já tem conta? entrar" na tela de Entrada, pra quem tem conta permanente sem sessão local. `bem-vindo/page.tsx` → "Já possuo meu espaço" → `/login`.
- [x] Tela de login (e-mail/senha ou magic link) pra esse caso. `login/actions.ts` — `entrarComSenha` (senha) e `enviarLinkMagico` (`signInWithOtp`, retorno via `/auth/callback`).
- [x] Logout: comportamento normal pra conta permanente; aviso forte pra conta anônima, por risco de perda permanente de dado. `perfil/SairButton.tsx` — conta anônima vê aviso + "criar conta antes" ou "sair mesmo assim"; conta permanente não muda.
...
- [x] **Portão de qualidade: nenhuma prática entra sem ter sido vivida** — mecanismo é o processo, não código: o Guilherme escreve/vive cada prática e me pede pra cadastrar uma por uma. Não é um item que "fecha" — é o funcionamento normal e contínuo da Biblioteca (sempre vai ter mais prática pra cadastrar).
- [ ] Rodar `scripts/recalcular-embeddings-biblioteca.mjs` uma vez pra corrigir a página semente (`embedding = null`). Cadastro de conteúdo novo é fluxo contínuo (`scripts/cadastrar-biblioteca.mjs`, calcula embedding, gera o SQL) — cada prática nova entra por decisão de voz/curadoria do Guilherme, não uma tarefa que "termina".
...
- [x] Respiração 4-7-8 — **feito** (item estava desatualizado aqui; ver Fase 6 abaixo, `/folego`, movimento contínuo, saída a qualquer momento). Confirmado funcionando pelo usuário. Pendente agora: cadastrar mais práticas (ver Fase 3, Biblioteca).
...
- [x] Cálculo de configuração HD implementado — `human-design-py` vendorizado (`services/ia/app/human_design_calc.py`, licença MIT do autor original, repo não tem LICENSE formal, só declaração no README). Pipeline de fuso horário de verdade: geopy/Nominatim (texto → lat/long) → timezonefinder (lat/long → fuso IANA) → pytz (fuso + data exata → offset histórico, trata virada de horário de verão). Fallbacks testados e funcionando: hora ausente → meio-dia + `horario_aproximado: true`; local ausente ou não geocodificável → America/Sao_Paulo + `local_aproximado: true` (achei em teste real que timeout de geocoding também precisava cair nesse fallback, não só "cidade não encontrada" — corrigido). Testado via `curl` local contra o endpoint real, incluindo autenticação. `apps/presenca/lib/humanDesign.ts` (espelha `lib/embed.ts`) chamado via `ctx.waitUntil` em `app/perfil/nascimento/actions.ts`, grava em `profiles.configuracao_hd`. **Falta**: testar o formulário `/perfil/nascimento` de ponta a ponta no navegador (não consegui — extensão do Chrome desconectada) e reimplantar `services/ia` no Railway com as dependências novas antes de valer em produção.
- [x] Autocomplete de cidade no campo de local de nascimento — resolve a ambiguidade do texto livre (ex: mais de uma "Santos" no Brasil) e o risco silencioso de geocoding achar uma cidade real mas errada. Provedor: Photon (komoot), gratuito, sem chave, feito pra autocomplete (ao contrário do Nominatim puro). Proxy autenticado em `app/api/geocoding/route.ts` (nunca chamado direto do browser, mesmo padrão de `/embed`/`/human-design`). Componente `CampoLocalidade.tsx` — debounce, dropdown, navegação por teclado; editar o texto depois de selecionar limpa a coordenada salva. Migration nova (`nascimento_latitude`/`nascimento_longitude` em `profiles`) — ao selecionar uma sugestão, o cálculo de HD pula o geocoding por texto inteiramente e usa a coordenada exata (`human_design_calc.py` refatorado com `offset_from_coordinates`, testado e confirmado idêntico ao caminho por texto pra mesma cidade). Sem selecionar sugestão, cai no texto livre de sempre (mesma tolerância de antes). Typecheck e build pro Workers (com as duas rotas de API) passaram limpos. **Falta**: testar a UI do autocomplete no navegador (mesma limitação de extensão desconectada).
...
- [x] Validado em `cf:preview` (runtime real do Cloudflare Workers) — streaming de ponta a ponta testado pelo usuário, funcionou.
...
- [x] Microsserviço Python — código pronto em `services/ia` (FastAPI, autenticado por `IA_API_KEY` compartilhada, nunca aberto). **Deployado no Railway** (`presenca-minha-production.up.railway.app`, Dockerfile builda a imagem com o modelo já cacheado). Endpoint `/embed` testado de ponta a ponta contra produção (multilingual-e5-small, 384 dim, prefixo passage/query). `IA_SERVICE_URL` preenchido nos `.env.local` do Presença e do Cuida. Endpoint `/human-design` implementado de verdade (ver Fase 7) — **reimplantado no Railway e testado em produção** (build precisou de `build-essential` no Dockerfile pra compilar `pyswisseph`; `/health` e `/human-design` autenticado responderam certo).
...
- [x] Termos da API Anthropic confirmados (`privacy.claude.com`, Commercial Terms + DPA): **não treinam com nosso conteúdo por padrão** — só mudaria com opt-in explícito no Development Partner Program ou envio de feedback avaliativo (thumbs up/down). Retenção operacional padrão: 30 dias (backend apaga input/output automaticamente); conteúdo sinalizado por violar a Política de Uso fica retido até 2 anos (scores de classificação, até 7 anos). Cliente retém direitos sobre Inputs e é dono dos Outputs. Anthropic = operadora no inventário LGPD.
...
- [x] **P5 Fase A — infraestrutura de sugestão de prática, EM PRODUÇÃO (2026-09-04), sem lente ainda.** Auditoria (2026-08-31) achou que o mecanismo presumido pelo documento original nunca existia; construído do zero: coluna `biblioteca.origin` (nullable, 6 valores válidos), `buscar_pratica_relevante` (match vetorial, `limit 3`), tools `sugerir_pratica`/`confirmar_pratica_mencionada` (primeira tool do projeto com ciclo completo `tool_use → tool_result → continuação`), `pratica_sugerida` em `caderno_entradas` só quando a menção é confirmada de verdade (não quando só oferecida ao modelo). 5 cenários testados em produção real. **Pool ainda quase vazio** — só 2 práticas de teste com `origin` preenchido; as 22 linhas originais de `biblioteca` seguem sem curadoria de origem, decisão consciente registrada em `integracao-presente-presenca-decisoes.md`. Fase B (lente como contexto secundário) meio bloqueada até esta fase acumular uso real.
- [ ] P5 Fase B — lente do dia como contexto secundário na sugestão de prática (nunca `Kin → prática`), depois da Fase A validada em uso real.
