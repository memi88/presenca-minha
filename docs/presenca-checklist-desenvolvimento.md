...
- [ ] Busca por proximidade semântica (`embedding <=>`) funcionando dentro do universo de cada usuário. **Bloqueado:** depende do microsserviço de IA estar no ar (`IA_SERVICE_URL` continua vazio — deploy pendente, fora do meu alcance).
...
- [ ] Migração da Meditação (`supabase/migrations/20260711134921_meditacao_biblioteca_slug.sql`) aplicada no banco — mesma pendência de sempre, precisa rodar no SQL Editor.
