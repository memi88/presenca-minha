...
import { editarExperiencia } from "./actions";
...
type Props = {
  ...
  tipoInicial: TipoExperiencia;
  descricaoInicial: string;
  estimativaFormatoInicial: string | null;
...
export function EditarForm({
  ...
  tipoInicial,
  descricaoInicial,
  estimativaFormatoInicial,
  etapasIniciais,
}: Props) {
  const [state, action, pending] = useActionState(editarExperiencia.bind(null, id), {});
  const [tipo, setTipo] = useState<TipoExperiencia>(tipoInicial);
  ...
  function adicionarEtapa() {
    setEtapas((prev) => [
      ...prev,
      { conteudo: "", tipoResposta: "texto", compartilhadaComEspecialista: false, opcoes: [] },
    ]);
  }
  ...
  return (
    <form action={action} className={styles.form}>
      ...
      <input className={styles.field} type="text" name="titulo" defaultValue={tituloInicial} required />
      ...
      <textarea className={styles.field} name="descricao" rows={3} defaultValue={descricaoInicial} required />
      ...
      <input
        ...
        defaultValue={estimativaFormatoInicial ?? ""}
